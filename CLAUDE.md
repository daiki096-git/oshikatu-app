...
* **git 運用**: harness で対応する変更は、**必ず main を基点に新しい作業ブランチを作成**してから実装し、完了後に **commit → push → PR作成（base main）→ GitHub上のレビュー（Claude等）の指摘対応** まで自動で行う。ブランチ名は依頼内容から自動生成する（**確認は取らない**）。**ブランチ名・commit・push・PR作成・修正のpush はすべて確認せず自動で進める**（main への最終マージは人が行う）。
* **ユーザーに確認するのは、判断で結論が変わる点（仕様・デザイン、および重要な技術判断）だけ**。ブランチ名・命名・インデントなど些末な点は確認せずデフォルトで進める。
